...
(function (root, factory) {
  ...
  function _placeTooltip(targetElement, tooltipLayer, arrowLayer, helperNumberLayer, hintMode) {
    ...
    var scrollElement = document.querySelector(this._introItems[this._currentStep].scrollElement);
    ...
    switch (currentTooltipPosition) {
      case 'top':
        ...
        var bottomOffset = (targetOffset.height + 20);

        if (scrollElement) {
          var rect = targetElement.getBoundingClientRect(),
            winHeight = _getWinSize(scrollElement).height,
            bottom = -1 * (rect.bottom - winHeight);
  ...
  function _showElement(targetElement) {
    ...
    if (oldHelperLayer != null) {
      ...
      // tooltipLayer.appendChild(bulletsLayer); // don't add it to parent
    ...
    if (!_elementInViewport(targetElement.element, scrollElement) && this._options.scrollToElement === true) {
      ...
      if (top < 0 || targetElement.element.clientHeight > winHeight) {
        if (scrollElement) {
          scrollElement.scrollTop = bottom + 70 + this._options.scrollPadding;
        }
        ...
        if (scrollElement) {
          scrollElement.scrollTop = top - this._options.scrollPadding;
        }
   ...
   * @param {Object} el
   */
  function _elementInViewport(el, id) {
    var rect = el.getBoundingClientRect();
    var sizes = _getWinSize(id);
    return (
      rect.top >= 0 &&
      rect.left >= 0 &&
      (rect.bottom + 80) <= sizes.height && // add 80 to get the text right
      rect.right <= sizes.innerWidth
    );
  }
